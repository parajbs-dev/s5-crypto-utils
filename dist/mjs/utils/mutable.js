 ...
 */
export async function encryptMutableBytes(data, key) {
 ...
 */
export async function decryptMutableBytes(data, key) {
    ...
    if (!checkPaddedBlock(data.length)) {
        throw new Error(`Expected parameter 'data' to be padded encrypted data, length was '${data.length}', nearest padded block is '${padFileSizeDefault(data.length)}'`);
    }
