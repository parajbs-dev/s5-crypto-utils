...
export class CryptoImplementation {
     ...
     */
    async deriveEd25519(masterKey, dataSeed, deriveLength) {
        await sodium.ready;
        let hasher;
        if (deriveLength) {
 ...
 */
export async function genKeyPairFromSeed(seed2) {
    await sodium.ready;
 ...
 */
export async function hashAll(...args) {
 ...
 */
export async function deriveChildSeed(masterSeed, tweakSeed) {
 ...
 */
export async function getDataKey(masterKey, dataSeed, deriveLength) {
    await sodium.ready;
 ...
 */
export async function testgen1() {
...
export function deriveKeyForPathSegments(_hiddenRootKey, pathSegments) {
    ...
    return deriveHashBlake3(deriveKeyForPathSegments(_hiddenRootKey, pathSegments.slice(0, pathSegments.length - 1)), [convert(Buffer.from(pathSegments[pathSegments.length - 1], 'utf8'))]);
}
